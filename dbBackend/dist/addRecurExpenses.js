...
const updateFtTables = async (expense) => {
    "use server";
    const { exp_date, exp_month, exp_year, user_id, updated_on, category_id } = expense;
    const conn = await connectDb();
    const getDiscretionaryQuery = readSQLScript("./sql/query/get_discretionary_for_category_id.sql");
    const getDiscretionaryParams = { category_id: category_id };
    const discretionaryResponse = await executeQuery(conn, getDiscretionaryQuery, getDiscretionaryParams);
    const discretionaryResult = discretionaryResponse.rows[0];
    const discretionary = discretionaryResult.discretionary;
    const updateParams = {
        exp_date,
        exp_month,
        exp_year,
    ...
    for (const fullFilePath of fullFilePaths) {
        ...
        const getResult = await executeQuery(conn, getQuery, updateParams);
        const resultRows = getResult.rows;
        const replaceQuery = readSQLScript(`${replaceFilePath}/${tblName}.sql`);
        for (const row of resultRows) {
            const replaceResult = await executeQuery(conn, replaceQuery, row);
        }
    }
};
const conn = await connectDb();
